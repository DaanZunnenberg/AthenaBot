...
# --- Lineage ----------------------------------------------------------------
# Adds: selective 3-of-5 graft of FiveBugfixes.py's bugfixes (FIX 2, 3, 5 only).
# Parent: EpsilonSharpen.py, fixes sourced from FiveBugfixes.py.
# ----------------------------------------------------------------------------
...
class MarketMaker:
    """PartialBugfixGraft: EpsilonSharpen (17.50/20, 10/16 outright #1 finishes) with 3 of the 5
    FiveBugfixes bug fixes applied -- FIX 2, 3, 5 -- and FIX 1 / FIX 4 deliberately left out.

    Context: FiveBugfixes (all 5 fixes) scored 16.50/20 on real HackerRank, DOWN from
    EpsilonSharpen's 17.50/20, with the regression concentrated in sessions EpsilonSharpen was
    previously winning outright against Fixed-Width competitors (tests 7, 9, 13, 14, 16
    all flipped from #1 to a loss -- see experimental/FiveBugfixes_Scores.md). Root-caused to
    FIX 1 (mid-blend) trading away real edge on a quote side that was never actually
    broken, and FIX 4 (margin netting) amplifying variance on top of that. Neither
    omitted fix is wrong on the math -- both close real bugs -- they're left out here
    because their net effect on the real scorecard was negative or too entangled with
    the regression to trust without a retuned, re-verified version.
    ...
    - FIX 2 (respond_to_fok): routes through `_size_for`'s inventory cap (previously
      ...
      matches `quote()`'s floor/ceil rounding and utilisation-scaled margin budget
      instead of the wider `_available_margin()`.
    - FIX 3 (_drawdown_severity): measures mark-to-market PnL (cash + open-position
      fair value), not raw cash alone -- previously a flat zero-EV buy/sell pair tripped
      severity 1.0 vs 0.0 purely from inventory sign.
    - FIX 5 (_portfolio_risk_score / _skew_for_side / _numeric_delta): deltas weighted
      by each underlying's one-step vol before combining (raw FED delta was ~265x AJR's,
      making the skew effectively FED-only); drops a side-independent squared term that
      was widening both sides instead of skewing; FED numeric-delta bump is now a full
      RATE_STRIKE_GRID step.

    OMITTED (left as EpsilonSharpen's original behavior, pending a retuned/re-verified fix):
    - FIX 1 (_mid_and_spreads): real bug (negative-edge quotes in the mid-confidence
      zone), but FiveBugfixes's fix discarded the trust-weighted shrinkage entirely and
      added a flat extra half-spread on both sides, giving up edge on the side that was
      never crossing fair value.
    - FIX 4 (on_trade margin netting): real bug (margin never releasing on a round trip
      to flat), but freeing that margin increases quoting size, plausibly amplifying
      variance in the combined FiveBugfixes submission.

    Ancestry (EpsilonSharpen <- DrawdownBreaker <- FlowRegimeTightening <- StableMerge <- Archived-A/PortfolioDeltaSkew): see
    experimental/EpsilonSharpen_Scores.md, DrawdownBreaker_Scores.md, FlowRegimeTightening_Scores.md for the full
    per-generation history -- three-zone confidence quoting, counterparty toxicity, a
    `_FlowRegime` favorable-markout signal, portfolio-level delta skew, capital-scale
    ramp, and hard solvency gates (`_available_margin`, `_worst_case_cash`, `_size_for`).
    """
    ...
    def _mid_and_spreads(self, option, counterparty_id):
        ...
        h_bid = max(0.005, base_h + skew_bid + t_b - tighten + widen - sharpen)
        h_ask = max(0.005, base_h + skew_ask + t_a - tighten + widen - sharpen)
