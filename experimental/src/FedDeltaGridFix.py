...
# --- Lineage ----------------------------------------------------------------
# Adds: bumps the FED numeric-delta finite-difference step to a full RATE_STRIKE_GRID step instead of a relative bump.
# Parent: AthenaBot/AthenaBot.py (the promoted submission, itself a copy of EpsilonSharpen.py / formerly EpsilonSharpen).
# ----------------------------------------------------------------------------

class MarketMaker:
    """Market-making bot for binary options on three correlated underlyings (a policy
    rate and two company valuations).

    Pricing: `warm_up` fits a joint model from historical data -- drift, idiosyncratic
    and shared-factor volatility, and cross-asset correlation for the two companies;
    up/down/reversion dynamics for the rate, found via a small grid search. Options are
    priced analytically (a probability-weighted lattice over future rate paths, combined
    with closed-form/quadrature integration over the companies' log-normal joint
    distribution), not simulated, so pricing is exact given the fitted parameters.

    Quoting: spread width and how much the quote trusts the fitted fair value both scale
    with a confidence score (how far the estimate sits from a neutral prior, weighted by
    how much data backs it) -- confident, well-supported estimates get tight, fair-value-
    centered quotes; sparse or borderline ones get pulled toward a defensive, wider quote.
    On top of that base spread, quotes are skewed by the portfolio's current
    correlation-aware risk exposure (so a trade that would concentrate risk gets a worse
    price, one that hedges existing exposure gets a better one), nudged by a slow,
    bounded read on each counterparty's historical adverse selection, and tightened
    slightly once a large sample of fills shows realized price moves have consistently
    been favorable rather than adverse.

    Risk management: a hard margin/inventory cap on every quote and FOK response keeps
    exposure bounded regardless of pricing error, and a separate, bounded circuit breaker
    widens spreads and shrinks size (without ever overriding the hard caps) if realized
    session PnL draws down past a conservative threshold.
    """
    ...
    class _BinaryOptionPricer:
        ...
        def _rate_lattice(market_parameters, rate0, steps):
            ...
            for _ in range(steps):
                ...
                for rate_value, rate_prob in lattice.items():
                    ...
                    stay = 1.0 - up - down
                    branches = (
                        (market_parameters.next_rate_value(rate_value, 1), rate_prob * up),
                        (market_parameters.next_rate_value(rate_value, -1), rate_prob * down),
                        (rate_value, rate_prob * stay),
                    )
        ...
        def price(cls, market_parameters, values, option, fast=False, lattice_cache=None):
            ...
            if steps <= 0:
                if steps == 0 and MarketMaker._SETTLEMENT_AFTER_ADVANCE:
                    steps = 1
                else:
    ...
    class _ParameterEstimator:
        ...
        def fit(cls, stats):
            try:
                ...
                return MarketMaker._FitResult(parameters)
            except Exception:
                return MarketMaker._FitResult(MarketMaker._default_market_parameters())

    @staticmethod
    def _default_market_parameters():
        return MarketParameters(ajarai_drift=0.0, ajarai_idio_std_dev=0.01, ajarai_rate_beta=0.0, ajarai_sector_beta=0.0, rate_down_probability=0.2, rate_reversion_strength=0.1, rate_up_probability=0.2, sector_std_dev=1.0, theriodic_drift=0.0, theriodic_idio_std_dev=0.01, theriodic_rate_beta=0.0, theriodic_sector_beta=0.0, rate_step=RATE_STRIKE_GRID, rate_target=2.0)

    _SETTLEMENT_AFTER_ADVANCE = True
    ...
    _DELTA_EPS = 0.01           # relative bump size for numeric delta (AJR/THR only)
    ...
    _C_LOW = 0.33    # confidence <  this -> wide zone, full trust in the neutral 0.5 prior
    _N_TARGET = 30.0
    ...
    # --- flow-regime detection: mirror-image of toxicity, bounded/capped, -----
    ...
    # the counterparty pool behaves like naive/low-information flow, without ever touching
    # the wide/low-confidence zone's safety-net spread.
    ...
    def __init__(self, underlying_initial_state, option_initial_state, cash_balance):
        ...
        self._stats = self._SufficientStats()
    ...
    def warm_up(self, market_history):
        try:
            ...
            self.estimated_parameters = self._default_market_parameters()
    ...
    def _refit(self):
        result = self._ParameterEstimator.fit(self._stats)
    ...
    def price_option_from_parameters(self, market_parameters, option):
        try:
            ...
            result = self._BinaryOptionPricer.price(market_parameters, values, option)
    ...
    def price_option(self, option):
        try:
            if not getattr(self, "_warmed_up", False) or self.estimated_parameters is None:
                return 0.5
            values = {u.underlying_id: u.value for u in self.underlying_state}
            result = self._BinaryOptionPricer.price(self.estimated_parameters, values, option)
            return result if math.isfinite(result) else 0.5
        except Exception:
            return 0.5
    ...
    def _numeric_delta(self, option, underlying_id, values):
        """Central-difference sensitivity of `option`'s price to `underlying_id`. FED
        only ever moves in discrete `RATE_STRIKE_GRID` steps, and price is a step
        function of it (a strike is crossed or it isn't) -- a relative sub-grid bump
        either lands inside one lattice cell (delta reads ~0, understating FED risk)
        or straddles a strike-crossing boundary (delta spuriously huge, since a full
        0-to-1 jump gets divided by a tiny denominator). Bumping by a full grid step
        instead measures the real one-step sensitivity the rate can actually realize.
        AJR/THR are continuous log-normal, so a small relative bump is well-behaved
        ...
        p_up = self._BinaryOptionPricer.price(self.estimated_parameters, up, option, fast=True)
        p_down = self._BinaryOptionPricer.price(self.estimated_parameters, down, option, fast=True)
    ...
    # --- covariance-aware portfolio risk score --------------------------------------
    def _portfolio_risk_score(self, delta_vector):
        ...
        # Reconstruct variances and covariances from estimated parameters
        ...
        # Portfolio variance calculation bridging cross-asset risk
    ...
    def _zone(self, confidence):
        ...
        if confidence < self._C_LOW:
            return 0.0, self._W_WIDE       # full trust in the neutral 0.5 prior, widest spread
    ...
    def _mid_and_spreads(self, option, counterparty_id):
        ...
        # (trust > 0) -- the wide zone's defensive safety-net spread for low-confidence
    ...
    def respond_to_fok(self, option, fok_order):
        ...
        Also enforces the same `_MAX_NET_PER_OPTION` inventory cap that `_size_for`
        already enforces on the RFQ/quote path -- previously only margin was checked here,
        so a counterparty submitting repeated FOKs against the same option could push net
        inventory past the stated per-option cap. Since a FOK is all-or-nothing, if the
        requested quantity would breach the cap the whole order is declined rather than
        partially filled."""
        try:
            ...
            net = self._net_position(option.option_id)
            new_net = net + (quantity if is_buy_side else -quantity)
            inventory_room = self._MAX_NET_PER_OPTION - abs(new_net) + 1
            if inventory_room <= 0:
                return False
