...
# --- Lineage ----------------------------------------------------------------
# Adds: compliance-audited, structurally reformatted copy (no functional/logic change).
# Parent: FokInventoryCapFix.py.
# ----------------------------------------------------------------------------

class MarketMaker:
    """AuditedRestructure: audited, restructured copy of FokInventoryCapFix. FokInventoryCapFix is CovarianceRisk (17.50/20,
    tied top real-HackerRank score) with its full stack left untouched, plus one targeted
    fix: `respond_to_fok` now enforces the same `_MAX_NET_PER_OPTION` inventory cap that
    `_size_for` already enforces on the RFQ/quote path. Previously `respond_to_fok` only
    checked `_available_margin`, so a counterparty issuing repeated FOK orders against the
    same option could push net inventory well past the stated per-option cap as long as
    margin allowed (independently verified: reachable up to ~20x the stated cap). This fix
    computes the same `inventory_room` bound `_size_for` uses
    (`_MAX_NET_PER_OPTION - abs(new_net) + 1`) and clamps the accepted FOK quantity to it,
    declining the fill (or none of it, since FOK is all-or-nothing) if the full requested
    quantity would breach the cap. No other logic changed: pricing, quoting, skew,
    toxicity, flow-regime, drawdown breaker, and margin math are all byte-identical to
    CovarianceRisk.

    DrawdownBreaker: two changes on top of FlowRegimeTightening. (1) narrows `_W_WIDE` (the wide/low-confidence
    fallback half-spread) from 0.25 to 0.18 to reduce a loss to flatter competitors while
    leaving the fallback in place for genuinely bad estimates. (2) adds a bounded,
    conservative per-session drawdown circuit breaker (`_drawdown_scale`) targeting an
    adverse-regime loss, layered on top of -- never weakening -- the existing hard
    solvency gates.

    FlowRegimeTightening: StableMerge + a lightweight, empirically-motivated flow-regime adaptation.

    Base engine (untouched): three-zone confidence quoting, counterparty toxicity/markout
    tracking, capital-scale ramp, and every hard solvency gate (`_available_margin`,
    `_worst_case_cash`, `_size_for`'s inventory/margin caps), plus portfolio-level
    cross-underlying delta skew in place of a single-option net-position skew. Zero
    recorded bankruptcies across every real HackerRank run of this lineage.

    Adds a `_FlowRegime` tracker: a bounded, capped, confidence-scaled "favorable markout"
    signal that mirrors the existing (adverse) toxicity tracker but in the opposite
    direction -- it accumulates the same per-trade markout observations already computed
    for toxicity, and when a substantial, non-adversarial sample of fills shows realized
    price movement has been favorable or flat (not adverse) after our fills, it narrows the
    effective spread width a small, hard-capped amount beyond the normal three-zone menu,
    only in the tight/mid confidence zones (never overriding the wide/low-confidence zone's
    safety net) and only once enough fills have accumulated to be statistically meaningful.
    """
