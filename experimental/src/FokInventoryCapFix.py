...
# --- Lineage ----------------------------------------------------------------
# Adds: enforces the same per-option inventory cap on respond_to_fok that quote() already enforced.
...
class MarketMaker:
    """FokInventoryCapFix: CovarianceRisk (17.50/20, tied top real-HackerRank score) with its full stack
    left untouched, plus one targeted fix: `respond_to_fok` now enforces the same
    `_MAX_NET_PER_OPTION` inventory cap that `_size_for` already enforces on the RFQ/quote
    path. Previously `respond_to_fok` only checked `_available_margin`, so a counterparty
    issuing repeated FOK orders against the same option could push net inventory well past
    the stated per-option cap as long as margin allowed (independently verified: reachable
    up to ~20x the stated cap). This fix computes the same `inventory_room` bound
    `_size_for` uses (`_MAX_NET_PER_OPTION - abs(new_net) + 1`) and clamps the accepted FOK
    quantity to it, declining the fill (or none of it, since FOK is all-or-nothing) if the
    full requested quantity would breach the cap. No other logic changed: pricing,
    quoting, skew, toxicity, flow-regime, drawdown breaker, and margin math are all
    byte-identical to CovarianceRisk.
    ...
    # --- Grafted from Archived-J: covariance-aware portfolio risk score --------------
    ...
    def respond_to_fok(self, option, fok_order):
        ...
        FIX (FokInventoryCapFix): also enforces the same `_MAX_NET_PER_OPTION` inventory cap that
        `_size_for` already enforces on the RFQ/quote path. `CovarianceRisk` (and its whole
        lineage) only margin-checked FOK acceptance here, so a counterparty submitting
        repeated FOKs against the same option could push net inventory past the stated
        per-option cap. Since a FOK is all-or-nothing, if the requested quantity would
        breach the cap the whole order is declined rather than partially filled."""
        try:
