...
# --- Lineage ----------------------------------------------------------------
# Adds: 5 verified quoting/risk bugfixes: mid-blend, FOK inventory cap, mark-to-market drawdown severity, and 2 more (see docstring).
...
class MarketMaker:
    """Debug.py: EpsilonSharpen (base: DrawdownBreaker, 17.50/20) with 5 quoting/risk bug fixes
    applied on top, each marked inline with a `FIX N:` comment. `experimental/EpsilonSharpen.py`
    is left untouched as the clean baseline to diff/harness-compare against; see
    `experimental/EpsilonSharpen_Scores.md` for that lineage's full history and rationale.

    FIX 1 (_mid_and_spreads): the trust-weighted midpoint blend toward 0.5 used to price
    one side of the quote through fair value itself (negative edge). Now always quotes
    around `fair`; uncertainty widens the spread symmetrically instead.
    FIX 2 (respond_to_fok): now routes through `_size_for`'s inventory cap (previously
    unenforced there, letting FOKs accumulate net position past `_MAX_NET_PER_OPTION`),
    and matches `quote()`'s floor/ceil rounding and utilisation-scaled margin budget.
    FIX 3 (_drawdown_severity): now measures mark-to-market PnL (cash + open-position
    fair value), not raw cash -- previously a flat, zero-EV buy and its symmetric sell
    tripped wildly different severities purely from inventory sign.
    FIX 4 (on_trade): margin is now recomputed from net position and a volume-weighted
    average entry price, not accumulated as gross per-trade debits -- previously a
    round trip back to flat still permanently consumed margin.
    FIX 5 (_portfolio_risk_score / _skew_for_side / _numeric_delta): deltas are now
    weighted by each underlying's own one-step volatility before combining (raw FED
    delta was ~50-260x AJR's, making the skew effectively FED-only); the skew calc
    drops a side-independent squared term that was widening both sides instead of
    skewing; and the FED numeric-delta bump is now a full rate-grid step instead of a
    sub-grid relative bump.

    Harness result after all 5 fixes (40 seeds, common random numbers, vs unmodified
    EpsilonSharpen): mean_score 2.0108 vs 2.0530 baseline (-0.0423, ~2%), 0/40 bankrupt both,
    19/4/17 win/tie/loss -- near parity locally despite fixing 5 real, independently
    verified bugs; see chat history / commit message for the full per-fix breakdown.
    """
    ...
    # --- epsilon-sharpen (EpsilonSharpen, SPECULATIVE) ------------------------------
    # See class docstring's PHASE 1 FINDING for the full reasoning: this fires only in
    # the highest-confidence zone (trust == 1.0) and shaves one minimum price increment
    # off the already-computed, zone-appropriate half-spread, never past fair value
    # itself. Zero-cost by construction -- it only converts a quote that was one cent
    # away from winning into one that wins, at a price we already judge favorable.
    _EPSILON_SHARPEN = 0.01
    ...
    _FLOW_REGIME_TIGHTEN_CAP = 0.02  # hard ceiling on extra spread narrowing (price units)
    ...
    def __init__(self, underlying_initial_state, option_initial_state, cash_balance):
        ...
        self._day_underlying_vols = {}
        self._day_index = 0
        self.estimated_parameters = None
        self._stats = _SufficientStats()
        self._warmed_up = False

        # FIX 4: volume-weighted avg entry price per option, for net (not gross) margin.
    ...
    def on_trade(self, option, price, quantity, counterparty_id):
        try:
            ...
            # FIX 4: margin from NET position + avg entry price, not gross debit sum --
            # a round trip to flat used to leave used_margin stuck nonzero (net=0 but
            # used_margin=5.00, verified). old_net/old_avg read before position updates.
            old_net = self._net_position(option.option_id)
            old_avg = self._avg_entry_price.get(option.option_id, 0.0)
            new_net = old_net + quantity
            if old_net == 0 or (quantity > 0) == (old_net > 0):
                # opening, or adding to a position in the same direction
                total_abs = abs(old_net) + abs(quantity)
                new_avg = (old_avg * abs(old_net) + price * abs(quantity)) / total_abs if total_abs > 0 else 0.0
            elif abs(quantity) <= abs(old_net):
                # partially or fully covering -- remaining position's cost basis is untouched
                new_avg = old_avg if new_net != 0 else 0.0
            else:
                # covered past zero -- the new (flipped-side) position's cost basis is this trade's price
                new_avg = price
            self._avg_entry_price[option.option_id] = new_avg
            new_margin = abs(new_net) * (new_avg if new_net > 0 else (1.0 - new_avg) if new_net < 0 else 0.0)
            old_margin = self._margin_by_option.get(option.option_id, 0.0)
            self._used_margin += new_margin - old_margin
            self._margin_by_option[option.option_id] = new_margin
        except Exception:
            pass
        if self._ENABLE_TOXICITY:
            try:
    ...
    def _settle_expired_positions(self, new_underlying_state, new_option_state):
        ...
        for option in self.active_option_state:
            ...
            self._avg_entry_price.pop(option.option_id, None)
            if quantity == 0:
                continue
    ...
    def _numeric_delta(self, option, underlying_id, values):
        ...
        if underlying_id == FED_FUNDS_RATE_UNDERLYING_ID:
            # FIX 5: rate lattice only moves in RATE_STRIKE_GRID steps -- the relative
            # 1% bump used for AJR/THR finite-differences a step function well inside a
            # single grid cell (verified: raw FED delta ~4.1 vs AJR ~0.016, ~265x gap).
            bump = RATE_STRIKE_GRID
        else:
    ...
    def _precompute_day_cache(self):
        ...
        self._day_underlying_vols = self._compute_underlying_vols()

    def _compute_underlying_vols(self):
        """FIX 5: one-step price/level vol per underlying, so FED/AJR/THR deltas are
        comparable before squaring in _portfolio_risk_score. AJR/THR reuse
        _company_moments' per-step log-return variance formula, scaled by level; FED
        (no lognormal vol field) uses rate_step * sqrt(p_up + p_down)."""
        if not self._warmed_up or self.estimated_parameters is None:
            return {}
        p = self.estimated_parameters
        values = {u.underlying_id: u.value for u in self.underlying_state}
        up = min(max(p.rate_up_probability, 0.0), 1.0)
        down = min(max(p.rate_down_probability, 0.0), 1.0)
        fed_sigma = p.rate_step * math.sqrt(max(up + down, 0.0))
        ajr_logret_sigma = math.sqrt(max(p.ajarai_sector_beta ** 2 * p.sector_std_dev ** 2 + p.ajarai_idio_std_dev ** 2, 0.0))
        thr_logret_sigma = math.sqrt(max(p.theriodic_sector_beta ** 2 * p.sector_std_dev ** 2 + p.theriodic_idio_std_dev ** 2, 0.0))
        ajr_level = values.get(AJARAI_UNDERLYING_ID, 0.0)
        thr_level = values.get(THERIODIC_UNDERLYING_ID, 0.0)
        return {
            FED_FUNDS_RATE_UNDERLYING_ID: max(fed_sigma, 1e-6),
            AJARAI_UNDERLYING_ID: max(ajr_level * ajr_logret_sigma, 1e-6),
            THERIODIC_UNDERLYING_ID: max(thr_level * thr_logret_sigma, 1e-6),
        }
    ...
    def _portfolio_risk_score(self, delta_vector):
        """FIX 5: raw deltas are in incompatible units across underlyings, making this
        score (and the skew derived from it) effectively FED-only. Weighting by each
        underlying's one-step vol first puts every term on a comparable scale."""
        vols = self._day_underlying_vols
        return sum((vols.get(uid, 1.0) * v) ** 2 for uid, v in delta_vector.items())
    ...
    def _skew_for_side(self, option, is_buy):
        ...
        # FIX 5: drop the direction**2*|d|**2 term (side-independent, was widening both
        # sides equally instead of skewing) -- keep only the 2*direction*<D,d> cross term.
        vols = self._day_underlying_vols
        cross = sum(
            (vols.get(uid, 1.0) ** 2) * self._day_portfolio_delta.get(uid, 0.0) * d
            for uid, d in option_delta.items()
        )
        skew = self._PORTFOLIO_RISK_K * 2.0 * direction * cross
    ...
    def _mid_and_spreads(self, option, counterparty_id):
        ...
        # FIX 1: always quote around our own best estimate -- blending toward 0.5 as
        # trust fell used to price one side through fair value itself (negative edge).
        # Uncertainty now widens the spread symmetrically instead of shifting the mid.
        blended_fair = fair
        uncertainty_width = (1.0 - trust) * (self._W_WIDE - self._W_TIGHT)
        ...
        # Epsilon-sharpen: only in the fully-trusted zone (trust == 1.0), and only after
        # every other term (skew, toxicity, flow-tighten, drawdown-widen) is already
        # applied -- shave one minimum increment off the resulting half-spread, still
        # floored at 0.005 so it can never cross into loss-making territory or bypass
        # the existing minimum-spread guard.
        sharpen = self._EPSILON_SHARPEN if trust >= 1.0 else 0.0
        h_bid = max(0.005, base_h + uncertainty_width + skew_bid + t_b - tighten + widen - sharpen)
        h_ask = max(0.005, base_h + uncertainty_width + skew_ask + t_a - tighten + widen - sharpen)
    ...
    def respond_to_fok(self, option, fok_order):
        ...
        try:
            ...
            # FIX 2: match quote()'s floor/ceil rounding (was bare round(), up to half
            # a cent more aggressive than the resting quote).
            if is_buy_side:
                our_price = math.floor(max(0.0, mid - h_bid) * 100.0) / 100.0
                if fok_order.price > our_price:
                    return False
            else:
                our_price = math.ceil(min(1.0, mid + h_ask) * 100.0) / 100.0
                if fok_order.price < our_price:
                    return False
            # FIX 2: route through quote()'s own _size_for -- was margin-only via the
            # wider _available_margin(), letting FOKs accumulate net position past the
            # inventory cap. No partial fill representable here, so oversized -> reject.
            allowed = self._size_for(option, fok_order.price, is_buy_side)
            return fok_order.quantity <= allowed
        except Exception:
            return False
    ...
    def _mark_to_market_pnl(self):
        """FIX 3: `self._cash` alone reflects trade-execution cash flow only, so a flat
        zero-EV buy and its symmetric sell tripped the drawdown breaker very differently
        (severity 1.0 vs 0.0, verified) purely from inventory sign. Adding back open
        positions' cached fair value gives real, sign-symmetric mark-to-market PnL."""
        position_value = 0.0
        for option_id, qty in self.position.option_quantity_by_option_id.items():
            if qty == 0:
                continue
            fair = self._day_cache.get(option_id, {}).get("P", 0.5)
            position_value += qty * fair
        return self._cash + position_value - self._starting_cash

    def _drawdown_severity(self):
        """0.0 (no drawdown response) to 1.0 (full, still-bounded response). Session PnL,
        not absolute cash level, is intentionally what's measured: a bot that started with
        ...
        if not self._ENABLE_DRAWDOWN_BREAKER or self._starting_cash <= 0:
            return 0.0
        pnl_frac = self._mark_to_market_pnl() / self._starting_cash
