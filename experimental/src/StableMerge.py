...
# --- Lineage ----------------------------------------------------------------
# Adds: first stable merge of the base three-zone engine with PortfolioDeltaSkew's cross-underlying skew.
# Parent: PortfolioDeltaSkew.py, on the Archived-A base.
# ----------------------------------------------------------------------------
...
class MarketMaker:
    """Mixture: Archived-A (16.60/20, best overall scorer) x PortfolioDeltaSkew (15.20/20).

    Base engine is Archived-A's three-zone confidence quoting (distance-from-0.5 x
    data-adequacy -> {tight, mid, wide} spread menu, anchored on the fixed-width values
    themselves) plus its counterparty-toxicity markout tracking and small-book capital
    scale -- this is the single highest-scoring architecture of the five source bots, with
    zero bankruptcies across 20 real HackerRank test cases and #1-of-N PnL rank in 8/16
    SCORED sessions.

    The one substitution: Archived-A's skew term only looked at *this option's* net
    position (`_SKEW_K * net`). PortfolioDeltaSkew instead prices skew off the whole book's
    correlated cross-underlying delta exposure (numeric delta of each open option w.r.t.
    each underlying it references, aggregated into a portfolio delta vector, skew
    proportional to the *change* in sum-of-squares risk score a fill would cause). That is
    a strictly more informed skew signal for a market with only three correlated
    underlyings and 2-leg spread contracts, where a single-option net-position skew can't
    see that two different options are both secretly long AJR-vs-THR risk. Combining it
    with the proven-safe toxicity/zone/capital-scale stack (left completely untouched)
    makes this the "safety-first" mixture: same hard solvency gates and same tested
    spread/zone logic as the top scorer, with a more accurate (not more aggressive) skew.
    """
    ...
    def _mid_and_spreads(self, option, counterparty_id):
        ...
        h_bid = max(0.005, base_h + skew_bid + t_b)
        h_ask = max(0.005, base_h + skew_ask + t_a)
