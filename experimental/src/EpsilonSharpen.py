...
# --- Lineage ----------------------------------------------------------------
# Adds: shaves one price increment off the half-spread at full confidence only (_EPSILON_SHARPEN).
...
class MarketMaker:
    """EpsilonSharpen: SPECULATIVE, UNVALIDATED experiment on top of DrawdownBreaker (17.50/20,
    current leader). Built to test a specific idea: can we design a mechanism that
    targets competing market-makers' PnL directly, without raising our own risk
    profile, rather than just "win more at our normal risk-adjusted threshold"?

    PHASE 1 FINDING (read this before trusting the framing below): `MarketMaker` never
    observes competitor identity, competitor quotes, or which trades we lost -- `quote`
    receives only `(option, counterparty_id)` where `counterparty_id` identifies the
    flow-submitter, not a rival MM; `respond_to_fok` receives only the FOK order terms;
    `on_trade`/`on_step_advance` report our own fills/state. RFQ routing ("goes to
    highest bid / lowest ask") happens entirely on the exchange side -- we get back a
    fill or nothing, never a signal of who else quoted or by how much we won/lost. The
    local replica confirms this independently: `sim/harness.py` states outright (top-of
    -file docstring) that it is a "single-agent harness [that] can't reproduce [ranking]
    without a competitor-MM model" and `sim/counterparties.py` only implements
    generic Noise/Informed/Mixed flow counterparties -- no Fixed Width / Stalemate
    Quoter / Lattice / Mongoose / Situational Unawareness archetype exists anywhere in
    this repo's code to inspect, target, or locally test against. Those names are only
    ever seen after the fact, in HackerRank's own post-session ranking text pasted into
    this repo's `*_Scores.md` files.

    CONCLUSION: there is no mechanical lever in this simulation that lets us "deny a fill
    to a specific competitor" as a distinct action from "win the fill ourselves at our
    existing risk-adjusted threshold." Winner-take-all RFQ routing means every fill we
    win is, tautologically, a fill some other bidder didn't win -- but we cannot detect,
    target, or verify that outcome differently from any other fill; and FOK acceptance
    is not confirmed anywhere in this codebase to be split multi-MM pro-rata (the FOK
    fill in `sim/harness.py` and `Bot.py`'s own `respond_to_fok` docstring both treat an
    accepted FOK as filled at the quantity we accept, with no split-share logic
    implemented or observable locally) -- so "capture more of the split share" is not
    something this codebase can exercise or validate either.

    Per the historical pattern documented in FlowRegimeTightening_Scores.md/StableMerge_Scores.md
    (AthenaBot loses outright to Fixed-Width competitors specifically in sessions where
    the realized edge is large and easy -- i.e. we are already comfortably profitable but
    lose the RFQ because a flat competitor quoted a hair better), the only genuinely
    "free" change available is: in the highest-confidence zone ONLY (`trust == 1.0`,
    where the fair-value estimate is already fully trusted, not blended toward 0.5), if
    our own risk-adjusted quote would still leave at least one whole minimum price
    increment (`_EPSILON_SHARPEN = 0.01`) of margin against the *already-accepted*
    zone-appropriate half-spread floor, shave the quoted price by that one increment
    toward the fair value. This is bounded to never move the quote past fair value
    itself (so it can never flip from a favorable to an unfavorable trade), never
    touches sizing, margin caps, toxicity, drawdown throttling, or any solvency gate, and
    never fires in the mid/wide confidence zones (where the fixed-width safety net for a
    less-trusted estimate must stay intact). It converts some near-miss losses into wins
    we would have taken anyway at our own threshold -- it does NOT lower our acceptance
    bar, and it does NOT specifically identify or react to Fixed Width vs. any other
    competitor, because the interface gives no way to do that.

    HONESTY CHECK (per task instructions): this is NOT a new mechanism. It is a
    narrowly-scoped, more conservative relabeling of "quote tighter when very confident,"
    which is a strategy class already explored by prior experimental bots (e.g. the
    `_W_TIGHT`/`_C_HIGH` zone itself, DrawdownBreaker's own `_W_WIDE` narrowing). The
    "denies a competitor a fill" framing is real in the sense that any RFQ we win was a
    fill some unseen competitor didn't get -- but that is equally true of every prior
    bot's ordinary edge-capture logic, so this does not add a distinguishable
    competitor-targeting capability beyond what "be more aggressive within our existing
    risk threshold, in the specific zone where we're most sure we're right" already is.

    DrawdownBreaker: SPECULATIVE, UNVALIDATED experiment on top of FlowRegimeTightening
    (17.20/20, current best real-HackerRank score). Two changes, both flagged by a prior
