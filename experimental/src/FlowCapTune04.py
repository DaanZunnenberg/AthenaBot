...
# --- Lineage ----------------------------------------------------------------
# Adds: widens _FLOW_REGIME_MIN_N 20 -> 12 and _FLOW_REGIME_TIGHTEN_CAP 0.02 -> 0.04 (single-constant tune).
...
class MarketMaker:
    """FlowCapTune04: on top of EpsilonSharpen (17.50/20 real-HackerRank leader), unchanged
    except two `_FlowRegime` constants: `_FLOW_REGIME_MIN_N` 20->12 and
    `_FLOW_REGIME_TIGHTEN_CAP` 0.02->0.04. See comments above `_ENABLE_FLOW_REGIME`
    for the rationale/bound analysis. Nothing else differs from EpsilonSharpen.

    EpsilonSharpen: SPECULATIVE experiment on DrawdownBreaker. Interface exposes no competitor
    identity/quotes (`quote(option, counterparty_id)`, `respond_to_fok(option, fok)`
    only) and `sim/harness.py`/`sim/counterparties.py` implement no competitor-MM
    model -- so "deny a competitor a fill" isn't mechanically distinct from "win at
    our existing risk-adjusted threshold." Given that, the only free change added:
    in the highest-confidence zone only (`trust == 1.0`), shave one price increment
    (`_EPSILON_SHARPEN = 0.01`) off the half-spread, never past fair value, never
    touching sizing/margin/toxicity/drawdown/solvency, never firing in mid/wide
    zones. Converts some near-miss losses into wins already judged favorable; does
    not lower the acceptance bar. This is a smaller version of "quote tighter when
    confident," a strategy class already present (`_W_TIGHT`/`_C_HIGH`, DrawdownBreaker's
    `_W_WIDE` narrowing) -- not a distinguishable competitor-targeting mechanism.

    DrawdownBreaker: on FlowRegimeTightening. (1) narrows `_W_WIDE` 0.25->0.18 (wide/low-confidence
    fallback) to cut Test-13 losses to flatter competitors. (2) adds a bounded
    per-session drawdown circuit breaker (`_drawdown_scale`) for the Test-20
    adverse-regime loss, layered on top of (never weakening) the hard solvency gates.

    FlowRegimeTightening: StableMerge (17.00/20) + a flow-regime adaptation.

    Base engine (untouched): Archived-A's three-zone confidence quoting, counterparty
    toxicity/markout tracking, capital-scale ramp, hard solvency gates
    (`_available_margin`, `_worst_case_cash`, `_size_for` caps), plus PortfolioDeltaSkew's
    portfolio-level cross-underlying delta skew. Zero recorded bankruptcies across
    this lineage's real HackerRank runs.

    Archived-K's hedge-size boost was independently found (30-session instrumented
    run) to fire on 1.10% of `_size_for` calls but produce bit-identical PnL,
    because counterparty-side quantity caps (8-10 units) already sit below our
    un-boosted `_MAX_NET_PER_OPTION=10` -- not the binding constraint. Pricing is
    independently reconfirmed exact (THEO max_error=0.0000; 0/205 martingale
    failures); the only real estimation gap is `warm_up`'s grid-search kappa
    resolution (~0.02-0.06 quantization, see debug/ESTIMATION.md), left untouched.

    Real pattern in HackerRank logs (StableMerge_Scores.md): AthenaBot loses outright
    to Fixed-Width 0.05/0.1/0.25 competitors in 7/16 SCORED sessions, worst when
    Fixed-Width's own PnL is large -- consistent with our three-zone spread being
    too conservative when realized edge is large and easy, exactly where a naive
    constant-spread quoter cleans up. Stalemate Quoter never beats us outright.

    Added `_FlowRegime`: a bounded, capped, confidence-scaled "favorable markout"
    signal mirroring the existing adverse-toxicity tracker in the opposite
    direction -- when fills show favorable-or-flat (not adverse) markouts over a
    large-enough sample, narrows the effective spread a small, hard-capped amount,
    only in tight/mid confidence zones (never overriding the wide-zone Fixed-
    Width-0.25 safety net), without touching solvency gates, sizing caps, or pricing.
    """
    ...
    # --- epsilon-sharpen (EpsilonSharpen, SPECULATIVE): fires only at trust==1.0, shaves
    # one min price increment off the half-spread, never past fair value. ------------
    _EPSILON_SHARPEN = 0.01

    # --- the fixed-width menu, used verbatim, never interpolated or invented -------
    _W_TIGHT = 0.05
    _W_MID = 0.10
    # SPECULATIVE (DrawdownBreaker_Scores.md): narrowed from 0.25 (FlowRegimeTightening's proven value)
    # to 0.18, still above _W_MID so the wide zone stays more defensive than mid.
    _W_WIDE = 0.18
    ...
    # --- flow-regime detection: mirror-image of toxicity, bounded/capped -- narrows
    # spread when markouts have been favorable-or-flat over a large-enough sample,
    # never touching the wide-zone safety-net spread.
    #
    # FlowCapTune04, SPECULATIVE: _FLOW_REGIME_MIN_N 20->12, _FLOW_REGIME_TIGHTEN_CAP
    # 0.02->0.04 (still inside the 0.05-wide _W_TIGHT zone; `_mid_and_spreads` still
    # floors every half-spread at 0.005). Rationale: in the worst real-HackerRank
    # losses (EpsilonSharpen_Scores.md Tests 6/8/10), a naive Fixed-Width competitor earns
    # large positive PnL while AthenaBot is flat/negative in the same sessions --
    # i.e. losing the fill-volume race to a static quoter in a low-toxicity regime.
    # The flow-regime signal already detects this (sustained favorable-or-flat
    # markout EMA) but was tuned conservatively (20-fill warmup, 2-cent cap) relative
    # to how few fills a short SCORED session produces. Stays additive on top of the
    # unchanged base spread/toxicity/skew/drawdown terms, only fires when trust > 0,
    # never on an adverse/flat-zero EMA. No sizing/margin/solvency logic touched.
    _ENABLE_FLOW_REGIME = True
    _FLOW_REGIME_MIN_N = 12        # global fill count before the signal is trusted at all
    _FLOW_REGIME_TIGHTEN_CAP = 0.04  # hard ceiling on extra spread narrowing (price units)
    _FLOW_REGIME_ALPHA = 0.03      # slow EMA, same spirit as _MARKOUT_ALPHA

    # --- drawdown circuit breaker (SPECULATIVE, on top of, never a substitute for,
    # the hard solvency gates): widens spreads / shrinks size caps once mark-to-market
    # session PnL (`self._cash - self._starting_cash`) draws down past a conservative
    # fraction of starting capital. Bounded response, recovers automatically as
    # `_cash` recovers -- no separate "tripped" latch. ------------------------------
